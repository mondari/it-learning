...
- 持久性(Durability)：事务一旦被提交了，那么对数据库中数据的改变就是永久性的，即便是在数据库系统遇到故障的情况下也不会丢失已提交事务的操作
...
- 不可重复读：指一个事务对数据库中的某个数据进行多次查询得到了不同的值，这是因为在查询过程中被其它事务修改并提交了。

  不可重复读和脏读的区别是，脏读是某一事务读取了另一个事务未提交的脏数据，而不可重复读则是读取了前一事务提交的数据。

- 幻读：指一个事务对数据库表中所有数据进行处理后，另一个事务向这批数据插入了新数据，由于新增数据没有被前一个事务处理，所以当前一个事务读取数据库所有数据后，会发现还有数据没有被处理，就像发生了幻觉一样，这就是幻读。
...
- 读未提交（Read Uncommitted）：事务可以读到其它事务未提交的记录。
- 读已提交（Read Committed）：事务只能读到其他事务已提交的记录。**可避免脏读的发生。**
- 可重复读（Repeatable Read）：这是 MySQL 默认隔离级别。事务无法读到其它事务已提交或未提交的记录，保证了事务在执行过程中读取到的数据前后一致。**可避免脏读、不可重复读的发生。**
- 串行|序列化（Serializable）：最高隔离级别。对于同一行记录，“写”会加“写锁”，“读”会加“读锁”。当出现读写锁冲突的时候，后一个的事务必须等前一个事务执行完成，才能继续执行。**可避免脏读、不可重复读、幻读的发生。**
...
8. 在子查询中，用 exists 代替 in 是一个好的选择。
   ...
   另外需要注意的是，IN 适合外表大的情况，EXISTS 适合内表大的情况。
...
10. 当只要一行数据时使用 LIMIT 1。

    数据库会在查找到一条数据后停止搜索，不会继续往后查询下一条符合条件的数据。

11. 使用 varchar 代替 char 。因为变长字段存储空间小，可以节省存储空间。
...
外表大，用 IN；内表大，用 EXISTS。

## 主从复制原理

主数据库将数据的改动操作写入到二进制日志Binary log中，从数据库会监听主数据库的二进制日志，如果监测到变化，就会启动一个IO线程向主数据库请求二进制日志，主数据库会为每个从数据库的IO线程启动一个Dump线程发送二进制日志，从数据库会将二进制日志保存到本地的中继日志Relay log，然后启动SQL线程读取中继日志进行同步操作

主从复制后，可以实现读写分离，主库写，从库读，降低数据库的读写压力。
