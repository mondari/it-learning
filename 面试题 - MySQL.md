...
- 不可重复读：指一个事务对数据库中的某个数据进行多次查询得到了不同的值，这是因为在查询过程中被其它事务修改并提交了。
  ...
  不可重复读和脏读的区别是，脏读是指一个事务读取了另一个事务未提交的脏数据，而不可重复读则是读取了另一个事务已提交的数据。

- 幻读：指一个事务对数据库表中所有数据进行处理后，另一个事务向这批数据插入了新数据，由于新增数据没有被当前事务处理，所以当前事务再次查询所有数据时，会发现还有数据没有被处理，就像发生了幻觉一样，这就是幻读。
