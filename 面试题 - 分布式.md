...
- P **分区容错性（比较难理解）**：这里的分区指的是网络分区。分布式系统中部分节点因为故障导致不连通，就会形成网络分区。如果一个数据只保存在一个节点上，一旦出现网络分区，系统无法返回正确的结果，这样的系统是没有容错性的（无法保证其分区容错性）。要想保证容错性，就要将数据复制到多个节点上，那么即使出现网络分区，数据仍然能在其他分区中读取。然而把数据复制到多个节点，就会带来**一致性的问题**，就是多个节点上面的数据可能是不一致的。也会带来**可用性的问题**。
