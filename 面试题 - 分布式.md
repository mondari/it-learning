...
CAP 定理，指的是在一个分布式系统中，一致性（Consistency）、可用性（Availability）、分区容错性（Partition-tolerance）这三个特性最多只能满足两个，不能三者兼得（通俗来说，就是“三选二定理”）。
...
- P **分区容错性（比较难理解）**：
  ...
  什么叫分区？分布式系统中部分节点因为故障导致不连通，就会形成一块一块的不连通区域，这就叫分区（画图就容易理解）。

  如果一个数据只保存在一个节点上，一旦出现分区服务就会出现异常，这样的系统容错性会很低。

  提高分区容错性的办法就是将一个数据复制到多个节点上，那么出现分区之后，这项数据仍然能在其他区中读取，容错性就提高了。然而把数据复制到多个节点，就会带来**一致性的问题**，就是多个节点上面的数据可能是不一致的。要保证一致，每次写操作就都要等待全部节点写成功，而这等待又会带来**可用性的问题**，无法在有限的时间内返回响应结果。
