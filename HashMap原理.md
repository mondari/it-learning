...
如果初始容量不为2的幂，HashMap会调用tableSizeFor方法，找到大于等于初始容量的最小2的幂。其代码如下
...
### *知识点：为什么HashMap中的HashCode是 `key.hashCode() ^ (h >>> 16)`
...
`h >>> 16` 是将哈希值无符号右移16位，高位补零。

## 扩容机制

### 知识点：扩容的条件？

当HashMap的大小超过阈值时，就会发生扩容（容量变为原来的两倍）和重新哈希（重新分布元素的位置）。阈值的大小就是HashMap容量和负载因子的乘积。
